...
const CONFIG_PATH = path.join(os.homedir(), '.bot0x');

const welcome = new Command('welcome')
  .description('Display welcome message and setup instructions')
  .option('--setup', 'Run interactive setup to create config file')
  .action(async (options) => {
    ...
    if (configExists) {
      ...
      if (!overwrite) {
        ...
        console.log(chalk.cyan('nano ~/.bot0x') + '\n');
        showAdditionalInfo();
        return;
      }
    }
    ...
    if (setupNow) {
      ...
      console.log(chalk.cyan('cp ' + chalk.italic('node_modules/bot0x/bot0x.config.example.json') + ' ~/.bot0x'));
      console.log(chalk.cyan('nano ~/.bot0x') + '\n');
      showAdditionalInfo();
    }
  });
...
async function setupConfigInteractive() {
  ...
  try {
    ...
    console.log(chalk.green('\n✅ Configuration file created successfully at ~/.bot0x\n'));
