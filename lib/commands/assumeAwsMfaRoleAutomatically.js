...
export async function assumeAwsMfaRoleAutomaticallyHandler(envArg) {
  ...
  // Get the role configuration from config file
  if (!config.aws || !config.aws.roles) {
    error('AWS roles configuration is missing in your config file.');
    error('Please update your ~/.bot0x/config.json to include role definitions.');
    return;
  }

  // Default to dev if the specified environment doesn't exist
  const roleKey = config.aws.roles[envArg] ? envArg : 'dev';

  if (!config.aws.roles[roleKey]) {
    error(`Role configuration for ${roleKey} not found in config file.`);
    error('Please check your ~/.bot0x/config.json file.');
    return;
  }

  const { env, arn } = config.aws.roles[roleKey];
