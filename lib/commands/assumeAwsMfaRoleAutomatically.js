...
export async function assumeAwsMfaRoleAutomaticallyHandler(envArg) {
  ...
  let assumeCmd = `aws sts assume-role --role-arn ${arn} --role-session-name ${username} --serial-number ${mfaSerial} --token-code ${token} --query "Credentials.[AccessKeyId,SecretAccessKey,SessionToken]" --output text`;
  ...
  // If the first attempt failed, try to wait for the next token
  if (code !== 0 && stderr && stderr.includes('invalid MFA one time pass code')) {
    ...
    if (newToken !== token) {
      ...
      assumeCmd = `aws sts assume-role --role-arn ${arn} --role-session-name ${username} --serial-number ${mfaSerial} --token-code ${newToken} --query "Credentials.[AccessKeyId,SecretAccessKey,SessionToken]" --output text`;
